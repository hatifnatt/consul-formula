...
consul_acl_support:
  test.configurable_test_state:
    ...
    - result: {{ c.config.acl.enabled|default(False) }}
    - changes: False
    - comment: "Consul ACL enabled: {{ c.config.acl.enabled|default(False) }}"
...
{{ policy.rules|json(indent=2) }}
        ...
        EOF
    - env:
      - CONSUL_HTTP_TOKEN: {{ c.config.acl.tokens.master }}
...
{%- set current_anon_policies = salt.cmd.shell("CONSUL_HTTP_TOKEN=" ~ c.config.acl.tokens.master ~ " consul acl token read -id='anonymous' 2>/dev/null | grep -o '\- .*$'", output_loglevel='quiet')|default('--- []', True)|load_yaml %}
...
{%- set agent_token_id = salt.cmd.shell("CONSUL_HTTP_TOKEN=" ~ c.config.acl.tokens.master ~ " consul acl token list 2>/dev/null | grep -B1 'Salt Created Agent Token' | grep -oP 'AccessorID\:\s+ \K(.*)'", output_loglevel='quiet')|default('', True) %}
{%- if agent_token_id %}
{%-   set current_agent_policies = salt.cmd.shell("CONSUL_HTTP_TOKEN=" ~ c.config.acl.tokens.master ~ " consul acl token read -id " ~ agent_token_id ~ " 2>/dev/null | grep -o '\- .*$'", output_loglevel='quiet')|default('--- []', True)|load_yaml %}
